    ...
    , d(new UserModelPrivate())
{
    ...
    */
    const QString facesDir = QStringLiteral("/usr/share/sddm/faces");
    const QString themeDir = QStringLiteral("/usr/share/sddm/themes");
    const QString currentTheme = QStringLiteral("breeze");
    ...
    const QString iconURI = QStringLiteral("file://%1").arg(QFile::exists(themeDefaultFace) ? themeDefaultFace : defaultFace);
    ...
    while ((current_pw = getpwent()) != nullptr) {
        ...
        */
        ...
        if (!needAllUsers && d->users.count() > 7) {
    ...
    // in nsswitch.conf(5).
    auto newEnd = std::unique(d->users.begin(), d->users.end(), [&](const UserPtr &u1, const UserPtr &u2) {
        return u1->name == u2->name;
    });
...
QString UserModel::lastUser() const
{
    ...
    return d->users.constFirst().get()->name;
}
