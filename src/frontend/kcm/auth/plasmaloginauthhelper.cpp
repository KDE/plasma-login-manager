...
{
    KUser plasmaloginUser("plasmalogin");
...
ActionReply PlasmaLoginAuthHelper::sync(const QVariantMap &args)
{
    ...
    if (!configLocation.exists()) {
        QDir().mkpath(configLocation.path());
    ...
    if (!fontConfigDir.exists()) {
        QDir().mkpath(fontConfigDir.path());
        chownPath(fontConfigDir.path());
    }
    ...
    auto createConfigFile = [&args, &homeDir](const QString &name) {
        ...
        if (file.open(QFile::WriteOnly | QFile::Truncate | QFile::Text, QFile::ReadOwner | QFile::WriteOwner | QFile::ReadGroup | QFile::ReadOther)) {
...
ActionReply PlasmaLoginAuthHelper::save(const QVariantMap &args)
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Truncate, QFile::ReadOwner | QFile::WriteOwner | QFile::ReadGroup | QFile::ReadOther)) {
        return ActionReply::HelperErrorReply();
    }
